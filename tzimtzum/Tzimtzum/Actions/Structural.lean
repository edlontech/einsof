...
`pending` and `challenges` are keyed by invocation but *owned* by an agent, so removing an
agent's entries is an `Option`-filtering traversal rather than V3's pointwise conjunction on
a curried relation. Both helpers are shared by `delegate`, `revoke` and `cascade_revoke` so
the three cannot drift, and both are the exact shape the Rust `VecMap` retain-loop refines.

The `ite` conditions here are deliberately *small* — a single equality on an opaque sort.
The Task 0 spike's finding 1 (architecture E8) is about `ite` on large `Prop`s, whose
`Classical.propDecidable` instance the discharge cascade then has to `whnf` at every step;
...
The helpers are `noncomputable def`s over a `Classical.propDecidable` `ite`, so the
discharge cascade cannot see through them on its own: `grind` stalls on the instance rather
than splitting it. These `@[simp]` lemmas are the API every Task 7 obligation touching
`pending`, `challenges` or `crossing_grants` goes through -- roughly ten conjuncts across
three actions, so paying for them once here is much cheaper than re-deriving them per goal. -/
