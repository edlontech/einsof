defmodule ExArgus.BudgetTest do
  ...
  no-debit-when-already-tainted rule. Budget is read from the projected state
  (`agent_budget`); absence == capacity == 16.
  """
  ...
  test "a bounded+conforming Sensitive endorsement debits 2 and adds no taint" do
    ...
    {:ok, _, _} =
      Instance.invoke_start(h, "a1", "summarize", "inv-1", true, %{"summarize" => true})
  ...
  test "return_endorsed is refused with :not_conforming when the verdict is false" do
    ...
    assert {:error, :not_conforming} = Instance.return_endorsed(h, "a1", "root", false)
  ...
  test "an endorsement against an already-tainted agent does not debit and keeps the taint" do
    h = Instance.new(bg())
    {:ok, _, _} = Instance.register_tool(h, "summarize")
    {:ok, _, _} = Instance.delegate(h, "root", "a1")
