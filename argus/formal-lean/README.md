...
TzimtzumV2 (Kav) spec. The end-to-end result is `implementation_sound`: **every reachable state of
the extracted kernel refines an abstract TzimtzumV2 state that satisfies all safety invariants.**

The Rust kernel is extracted to Lean with Charon + Aeneas (`scripts/charon-aeneas-extract.sh` →
`ArgusLean/Generated/ArgusKernel.lean`); this library proves the extracted model refines the spec.
...
Lean 4.30 (pinned via `lean-toolchain`). After a toolchain change run `lake exe cache get` first.
...
Layering is strict: `Generated` ← `Bridging` ← `Unified`. Nothing in `Bridging` or `Generated`
depends on `Unified`.

## What `implementation_sound` proves — and what it doesn't

For every reachable concrete `KernelState c`, there is an abstract state `a` with `R c bg a` and
`Tzimtzum.allInv a` (every TzimtzumV2 safety invariant holds). It is **forward simulation composed
with the Kav soundness theorem** — no SMT solver in the loop.

It is sound **modulo** the trusted extractor and two explicit, honest assumptions; it does **not**
prove the hand-written Rust source, the oracles, or the SPIFFE/STS mesh + Elixir adapter are correct.
State the extractor + assumptions whenever claiming end-to-end soundness.
...
Two assumptions are **hypotheses** of `implementation_sound` (discharged by the caller, not axioms):

- **`CapacityOK`** — the `Vec`-capacity bounds (`… .length < Usize.max`) each transition needs. There
  is no resource model here to discharge them.
- **`OracleFidelity`** — the runtime `ContentGate` / `Authorizer` / `Conformance` oracles agree with
  the abstract state's oracle fields.

There are **no proof `sorry`s** in the refinement; the verification is kernel-checked, no solver.
